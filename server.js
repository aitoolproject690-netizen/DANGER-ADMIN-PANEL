...
const defaultCustomerExpiry = new Date(Date.now() + 30*24*60*60*1000).toISOString();
const defaultSettings = {
  panel_name: "DANGER ADMIN PANEL",
  default_game: "My APK",
  maintenance_mode: "0",
  customer_panel_expires_at: defaultCustomerExpiry
};
...
function auth(req,res,next) {
  ...
  if (panelExpired(user)) {
    return res.status(403).json({error:"DANGER customer panel access has expired",code:"PANEL_EXPIRED"});
  }
...
function adminOnly(req,res,next) {
  ...
  if (panelExpired(user)) return res.status(403).json({error:"DANGER customer panel access has expired",code:"PANEL_EXPIRED"});
  req.currentUser = user;
  req.session.user = {...req.session.user, ...user, max_device_limit:user.max_device_limit || 2000};
  next();
}
function panelExpired(user) {
  ...
  if (user.role === "reseller") return Boolean(user.panel_expires_at && new Date(user.panel_expires_at) <= new Date());
  if (user.role === "admin") {
    const expiry = db.prepare("SELECT value FROM settings WHERE key='customer_panel_expires_at'").get()?.value;
    return Boolean(expiry && new Date(expiry) <= new Date());
  }
  return false;
}
function makeKey() {
  return "DANGER-" + crypto.randomBytes(9).toString("base64url").toUpperCase();
}
...
app.post("/api/login",(req,res)=>{
  ...
  if (panelExpired(u)) return res.status(403).json({error:"DANGER customer panel access has expired",code:"PANEL_EXPIRED"});
...
app.get("/api/me",(req,res)=>{
  ...
  if (panelExpired(user)) return res.status(403).json({user:null,error:"DANGER customer panel access has expired",code:"PANEL_EXPIRED"});
  if (user.role === "reseller" && db.prepare("SELECT value FROM settings WHERE key='maintenance_mode'").get()?.value === "1") {
